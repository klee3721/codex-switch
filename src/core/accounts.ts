...
import type { Account, AppState, AuthTokens, SwitchResult } from './types'
...
function computeAuthSignature(tokens: AuthTokens) {
  const seed = [tokens.authMode ?? '', tokens.accountId ?? '', tokens.accessToken].join('\n')
...
async function resolveUsageSnapshot(profileDir: string) {
  try {
...
async function enrichMissingSignatures(accounts: Account[]) {
  let changed = false
  const next = await Promise.all(
    accounts.map(async (account) => {
      if (account.authSignature && account.email) return account
      try {
...
export async function ensureCurrentCodexLinked(preferredLabel = CURRENT_ACCOUNT_LABEL): Promise<EnsureCurrentLinkResult> {
  ...
  const enriched = await enrichMissingSignatures(state.accounts)
  const workingAccounts = enriched.accounts
  ...
  if (matched) {
    ...
    const activeChanged = state.activeAccountId !== nextMatched.id
    if (activeChanged || enriched.changed || nextMatched !== matched) {
      await saveState({
